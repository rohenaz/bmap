...
describe('aip', () => {
    ...
    test('parse tx', async () => {
        ...
        expect(dataObj.AIP && dataObj.AIP[0].algorithm).toEqual('BITCOIN_ECDSA')
        expect(dataObj.AIP && dataObj.AIP[0].address).toEqual(
            '134a6TXxzgQ9Az3w8BcvgdZyA5UqRL89da'
        )
        expect(dataObj.AIP && dataObj.AIP[0].signature).toEqual(
            'H+lubfcz5Z2oG8B7HwmP8Z+tALP+KNOPgedo7UTXwW8LBpMkgCgatCdpvbtf7wZZQSIMz83emmAvVS4S3F5X1wo='
        )
        expect(dataObj.AIP && dataObj.AIP[0].verified).toEqual(true)
    })
    ...
    test('indexed tx', async () => {
        ...
        expect(dataObj.AIP && dataObj.AIP[0].index).toEqual([
            0, 1, 2, 3, 4, 5, 6,
        ])
        expect(dataObj.AIP && dataObj.AIP[0].verified).toEqual(true)
    })
