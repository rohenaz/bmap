...
describe('bitcom hashed', () => {
    ...
    test('parse invalid tx', async () => {
        ...
        const tx = {} as BobTx
        await expect(
            BITCOM_HASHED.handler({ dataObj, cell, tx })
    ...
    test('parse tx - out 1', async () => {
        ...
        expect(
            dataObj.BITCOM_HASHED && typeof dataObj.BITCOM_HASHED[0]
        ).toEqual('object')
        expect(
            dataObj.BITCOM_HASHED && dataObj.BITCOM_HASHED[0].paymail
        ).toEqual('luke@relayx.io')

        // TODO: The next 2 expectations are incorrect!
        expect(
            dataObj.BITCOM_HASHED && dataObj.BITCOM_HASHED[0].pubkey
        ).toEqual('Ar2rUhi8D442diOa26+PlcroBf2Itrj35Uuc6ks82l0J')
        expect(
            dataObj.BITCOM_HASHED && dataObj.BITCOM_HASHED[0].signature
        ).toEqual(
        ...
        expect(
            dataObj.BITCOM_HASHED && dataObj.BITCOM_HASHED[0].verified
        ).toEqual(false)
    })
